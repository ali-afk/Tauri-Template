 ...
 * ```
 *
 * @see src/lib/data/design-tokens.ts for easing/duration values
 */
 ...
 */
const CSS_TO_SVELTE_MAP: Record<string, EasingFunction> = {
	"ease-in": quadIn,
	"ease-out": quadOut,
...
/**
 * Lookup table for all Svelte easing functions by name
 */
const SVELTE_EASINGS: Record<string, EasingFunction> = {
	backIn,
	backInOut,
	backOut,
	bounceIn,
 ...
 */
function getEasing(queriedElement?: HTMLElement): EasingFunction {
	const raw = queryCssProperty("--transition-easing", queriedElement);
	if (!raw) throw Error("var(--transition-easing) is invalid!");

	// Check if it's a CSS keyword mappable to a Svelte easing function
	// OR Check if Svelte has a built-in easing with this name
	const matchedEasing = CSS_TO_SVELTE_MAP[raw] ?? SVELTE_EASINGS[raw];
	if (matchedEasing) {
		return matchedEasing;
	}

	// Check if it's a CSS keyword mappable to bezier coords, or parse as cubic-bezier
 ...
 *
 */
export function parseBezierCoords(
	bezier: string,
): [number, number, number, number] {
	const regex =
		/cubic-bezier\(\s*(0|1|0?\.\d+)\s*,\s*(-?\d+(?:\.\d+)?)\s*,\s*(0|1|0?\.\d+)\s*,\s*(-?\d+(?:\.\d+)?)\s*\)/;
	...
	match.slice(1, 5).forEach((coord) => {
		const parsedCoord = Number(coord);
		const isValidNum = !Number.isNaN(parsedCoord);
		if (!isValidNum) {
			throw Error(`Invalid coord: ${coord} in bezier: ${bezier}`);
		}
		coords.push(parsedCoord);
	});
