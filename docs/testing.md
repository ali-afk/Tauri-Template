...
(`runner: bun`). Tested via vitest at
`scripts/commit-msg/__tests__/lint-title.test.ts` using only Bun APIs:

The script exports a `types` array and wraps its main logic in
`import.meta.main` so it can be imported as a module by the test without running
the argv-dependent code.
