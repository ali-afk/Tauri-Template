...
`design-tokens.ts` and generates `gen.css`. This auto-runs via `vite-plugin-run`
on dev start (watches `design-tokens.ts` changes) and via `bun gen`.
`variables.css` then aliases fluid font sizes and spacing to Open Props values
and adds responsive overrides.

### Auto-Generated CSS: `gen.css`

Triggered by two mechanisms:

- **Dev start:** `vite-plugin-run` watches `src/lib/data/design-tokens.ts` for
  changes
- **Manual:** `bun gen` runs `scripts/gen/index.ts`

Output: `src/lib/styles/gen.css` — do NOT edit directly. Contains `@property`
declarations (for animatable custom properties) and `:root {}` block with real
values.
...
Font sizes and spacing are fluid via `clamp()` — no per-component media queries
needed.
...
Vite resolves path aliases (e.g. `$assets`) in CSS `url()` — don't replace with
relative paths.
...
Fluid values handle most responsiveness automatically. Use explicit media
queries only for structural layout changes (column count, direction, etc.).

**Breakpoints** (from Open Props via `design-tokens.ts`): `--breakpoint-sm`,
...
This is done in `variables.css` with `@media (max-width: ...)` overrides.
...
**Reserved substrings** — avoid these in unrelated class names: `row`, `stack`,
`title`, `card-grid`, `center`, `lift`

## GPU Optimization

`.card` and `.btn` include `backface-visibility: hidden` and
`transform: translateZ(0)` for smoother transitions. Disable if you notice
blurry text.

## Loading Screen

`loading.css` provides a shimmer animation (`class="loading-item"`) shown in the
root layout while the Tauri backend initializes app config. Removed after IPC
completes.
...
| File                | Purpose                                                           |
| ------------------- | ----------------------------------------------------------------- |
| `resets.css`        | Box-sizing, margin, font smoothing, 62.5% base font               |
| `gen.css`           | Auto-generated `@property` + `:root` from design tokens           |
| `variables.css`     | Fluid tokens, semantic aliases, responsive overrides              |
| `layout.css`        | Global transitions, body, sticky header, link/button resets       |
| `loading.css`       | Initial shimmer animation while backend initializes               |
| `utility.css`       | Composable classes: stack, row, center, card-grid, wrapper, title |
| `interactive.css`   | Auto-contrast system: `.card`, `.btn`, `.lift`                    |
| `accessibility.css` | `prefers-reduced-motion`, focus styles, external link icons       |
...
See [`docs/color-system.md`](./color-system.md) for the full explanation, math,
and debugging guide.
